...
public class CPU {
    ...
    public CPU() {
        ...
        MP = new MainMemory();
        cache = new CacheController(MP, 16); 
    ...
    public void runFourthSubcycle() {
     ...
     // --- INTEGRAÇÃO COM A CACHE ---
        // Aqui aplicamos a máscara (0x0FFF) para garantir que apenas os 
        // 12 bits de endereço sejam usados, ignorando o Opcode.
        if (signal_RD == 1) {
    ...
    private void saveCPUState() {
        ...
        logWriter.printf("CLOCK %d | MPC: %d | MIR: %d | Cache Hits: %d | Cache Misses: %d\n", 
            clock.getClockCounter(), MPC.get(), MIR.get(), cache.hits, cache.misses);
    }
    ...
    public void nextMacro() {
        ...
        // (MPC 0 é o estado inicial de busca de qualquer instrução)
        do {
            nextMicro(); 
            safetyCount++;
